...
![Fig. S. Pi1 analysis ascertaining neuron stimulation eQTLs in each other. An element in the ith row and the jth column indicates we used the P values from the jth condition and the eQTL from the ith condition.]
...
![Fig 7C. PIPs of high confidence genes in SZ cTWAS](assets/SCZ_gene_PIP.png)
...
![Fig S. eQTL plot for CPT1C in NMglut](assets/CPT1C_eqtl.png)
...
ggplot(dat, aes(y=trait,x=context,size=h2g,color=enrichment)) + 
  geom_point() + theme_bw() + scale_size(range=c(.1,7)) + 
...
ggplot(dat, aes(x=trait,y=h2g,fill=type)) + 
  ...
  geom_text(aes(label=scales::percent(h2g, accuracy = 0.1)),position = position_dodge(width=0.5),vjust=-0.5) + 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), legend.position = "top")
#ggsave("../Documents/neuron_stim/figure_pdf/Fig8D_h2g_ctwas.pdf", height=5.2,width=3.5,units="in")
```
