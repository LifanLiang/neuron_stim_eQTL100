...
barplot(post.sum$x,horiz = T,names.arg = post.sum$Group.1,las=2)
```
...
```{r,echo=F}
barplot(eigen.res$vectors[,1],names.arg = rownames(pat),las=2)
```

Eigen vector of 7.09% PVE

```{r,echo=F}
barplot(eigen.res$vectors[,2],names.arg = rownames(pat),las=2)
```
