...
```{r,echo=F}
barplot(sapply(params,function(x){sum(x$group_pve[-1])/x$total_pve}),las=2
        ,ylab="Total proportion of H2G",col="deepskyblue")
```
...
colnames(dat)[1] <- "h2g"
ggplot(dat,aes(x=condition,y=trait)) + geom_count(aes(color = h2g, size=enrichment)) +guides(color = 'legend') + theme_minimal() + scale_size(range=c(.1,20)) + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
```
