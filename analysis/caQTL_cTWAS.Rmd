...
tab <- npglut.table[,c(1,4,3,2)]
rownames(tab) <- NULL
tab[order(tab$PIP,decreasing = T),] %>%
  kbl(caption = paste(nrow(tab),"SNPs with a sum of PIP>0.8 in npglut")) %>%
  kable_styling()
```

```{r, echo=F}
tab <- GABA.table[,c(1,4,3,2)]
rownames(tab) <- NULL
tab[order(tab$PIP,decreasing = T),] %>%
...
tab <- GABA.table[,c(1,3,2)]
rownames(tab) <- NULL
tab[order(tab$aggregate_PIP,decreasing = T),] %>%
  kbl(caption = paste(nrow(tab),"SNPs with a sum of PIP>0.8 in GABA")) %>%
  kable_styling()
```

```{r, echo=F}
tab <- npglut.table[,c(1,3,2)]
rownames(tab) <- NULL
tab[order(tab$aggregate_PIP,decreasing = T),] %>%
  kbl(caption = paste(nrow(tab),"SNPs with a sum of PIP>0.8 in npglut")) %>%
  kable_styling()
...
Aggregate PIPs of peaks by locus. Locus tag dictates locus location. The first number indicates the chromosome and the second number indicates the ordered position of the locus in the chromosome.
...
hist(GABA.locusPIP$PIP_sum, breaks = 30, main="GABA time point specific PIP aggregated on locus level")
...
tab <- npglut.table
rownames(tab) <- NULL
tab$locus_tag <- sapply(strsplit(tab$locus_tag," "), function(x){paste0(x[1],"_",x[2])})
tab[order(tab$PIP,decreasing = T),] %>%
  kbl(caption = paste(nrow(tab),"SNPs with a sum of PIP>0.8 in npglut")) %>%
  kable_styling() %>%
  scroll_box(height="400px")
