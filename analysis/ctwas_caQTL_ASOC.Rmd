...
```{r,echo=F,message=F}
library(dplyr)
res5 <- readRDS("data/SCZ_cTWAS_peak_target_gene.rds")
res5[res5=="NA"] <- ""
res5[,c(1,2,13,15,14,16,17)] %>%
  DT::datatable() %>%
...
![](assets/ctwas_SCZ_eqtl_vs_caqtl_filtercs.png)
...
eqtl.range <- makeGRangesFromDataFrame(eres[eres$id %in% cand,1:3], 
...
peak.range <- makeGRangesFromDataFrame(res[res$id %in% cand,1:3], 
                                       seqnames.field = "chrom",
                                       start.field = "pos",
                                       end.field = "pos")
...
DT::datatable(ctwas.match[order(ctwas.match$distance),])
```
