...
p <- venn.diagram(list(cPeaks=peaks,eQTL=eres4$genename),fill=myCol,alpha=0.8,
             filename=NULL,lty="blank",cex=1.8,cat.pos=c(150,-150))
grid.newpage()
grid.draw(p)
dev.off()
```

Alternative with PIP threshold of 50%.
...
venn.diagram(list(cPeaks=peaks,eQTL=eres4$genename),fill=myCol,alpha=0.8,
             filename="docs/assets/ctwas_peaks_vs_gene.png",
             imagetype = "png",lty="blank",cex=3.5)
#pdf(file="../Documents/neuron_stim/figure_pdf/FigS_venn_ctwas_caqtl_vs_eqtl_pip5.pdf",height = 3,width=3)
p <- venn.diagram(list(cPeaks=peaks,eQTL=eres4$genename),fill=myCol,alpha=0.8,
             filename=NULL,lty="blank",cex=3.5,cat.pos=c(150,-150))
grid.newpage()
grid.draw(p)
#dev.off()
```
