...
```{r, echo=F}
pi1.gtex <- read.table("data/pi1_with_gtex.txt")
pheatmap(pi1.gtex[c(1,4,7,2,5,8,3,6,9),], cluster_rows = F, cluster_cols = F, angle_col = 315, display_numbers = values)
```
...
```{r,echo=F}
pi1.da <- read.table("data/pi1_vs_jb_dopamin.txt")
pheatmap(pi1.da[c(1,4,7,2,5,8,3,6,9),], cluster_rows = F, cluster_cols = F, angle_col = 315, display_numbers = values)
```
...
## Most recent cTWAS results on all the traits with eQTL, caQTL, and ASoC are [here](updated_ctwas.html).
